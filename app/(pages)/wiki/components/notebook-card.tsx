...
export const NotebookCard = ({
  ...
  return (
    <Link
      ...
      className="group grid h-full grid-rows-[auto_1fr] overflow-hidden rounded-[1.5rem] border border-zinc-800 bg-main outline-none transition duration-300 hover:-translate-y-0.5 hover:border-zinc-600 hover:bg-zinc-900/10 focus-visible:ring-2 focus-visible:ring-brand-400 focus-visible:ring-offset-4 focus-visible:ring-offset-main motion-reduce:transform-none motion-reduce:transition-none"
    >
      <div className="relative mx-3 mt-3 aspect-[16/9] overflow-hidden rounded-[1.1rem] bg-zinc-900">
        {notebook.cover ? (
          <WikiImage
          ...
          <div
            aria-hidden="true"
            className="grid size-full place-items-center bg-zinc-900/60"
          >
            <IconNotes className="size-10 text-zinc-700" stroke={1.2} />
          </div>
        )}
      </div>
      ...
      <div className="flex min-w-0 items-end justify-between gap-4 p-4 sm:p-5">
        <div className="flex min-h-36 min-w-0 flex-1 flex-col">
          <h2 className="text-balance text-xl font-bold leading-tight tracking-[-0.025em] text-zinc-50 transition-colors group-hover:text-brand-200 group-focus-visible:text-brand-200 sm:text-2xl">
            {notebook.title}
          </h2>
          ...
          <p className="mt-auto pt-4 font-mono text-[0.68rem] uppercase tracking-[0.12em] text-zinc-300">
